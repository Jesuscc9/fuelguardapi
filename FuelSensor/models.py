...
class FuelSensor(models.Model):
    truck = models.OneToOneField(
        Truck, on_delete=models.CASCADE, related_name="fuel_sensor"
    )
    sensor_type = models.CharField(
        max_length=50, choices=[("pressure", "Pressure"), ("level", "Level")]
    )
    certification = models.CharField(
        max_length=50, choices=[("atex", "ATEX"), ("iecex", "IECEx")]
    )
...
class FuelData(models.Model):
    sensor = models.ForeignKey(
        FuelSensor, on_delete=models.CASCADE, related_name="fuel_data"
    )
...
class Geofence(models.Model):
    ...
    lat = models.CharField(max_length=50)  # Latitud como cadena de texto
    lng = models.CharField(max_length=50)  # Longitud como cadena de texto
    radius = models.PositiveIntegerField()  # Radio como número entero positivo
...
class FuelHistory(models.Model):
    truck = models.ForeignKey(
        Truck, on_delete=models.CASCADE, related_name="fuel_history"
    )
